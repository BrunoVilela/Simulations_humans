...
## 2c) Phylogenetic isolation -- Sum of evolutionary distinctiveness
	...
	# ED - Summed evolutionary distinctiveness
		...
		Evolutionary_distinctiveness <- as.vector(unlist(lapply(Evolutionary_distinctiveness, sum)))
...
## 3a) Branch lengths 
	...
	## 3a.1 -- Sum of branch lengths divided by species richness
		...
		Branch_Lengths <- as.vector(unlist(lapply(Branch_Lengths, mean)))
...
## 4a) Tree topology -- Branching symmetry and distribution
	...
	#Gamma index
     ...
     gamma_list <- ltt(all_trees_gamma, gamma=TRUE, plot=FALSE)
  	 tip_length <- 1254
  	 rep_length <- length(gamma_list)
  	 
  	 lineages_through_time <- 	matrix(rep(NA, tip_length * rep_length),  tip_length, rep_length, byrow=TRUE)
  	time_steps <- matrix(rep(NA, tip_length * rep_length),  tip_length, rep_length, byrow=TRUE)
  	gamma <- rep(NA, rep_length)
  	gamma_p_value <-  rep(NA, rep_length)
