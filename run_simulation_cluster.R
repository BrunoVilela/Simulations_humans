...
if(chosen_combo[[2]] == "Extinct" ) {P.extinction  <- parameters(0, 0, 0.5, 0.5, "For", "Dom", "For", "Dom") } else {P.extinction  <- parameters(0,0,0,0, "For", "Dom", "For", "Dom")}

if(chosen_combo[[2]] == "Diffusion" ) {P.diffusion <- parameters(0.1, 0.1, 0.1, 0.1, "For", "Dom", "For", "Dom") } else {P.diffusion <- parameters(0,0,0,0, "For", "Dom", "For", "Dom")}
...
save(myOut, file = paste("myOut replicate_", replicate_cycle, " function_combination_type_", combo_number,Sys.time()," Results.Rdata", sep=""))
    }
