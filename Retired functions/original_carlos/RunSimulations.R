rm(list=ls())
source("/Users/CarlosMacPro/Dropbox/Projects/CulturalEvoSimulations/simulations_carlos.R")
...
P.Arisal <- as.data.frame(matrix(c(0.05, 0, 0, 0.05), 2, 2, byrow = T)) # P of coming up with a novel subsistence mode
...
P.diffusion <- as.data.frame(matrix(c(0, 0, 0, 0), 2, 2, byrow = T)) # P of diffusing your trait to a particular target
...
P.TakeOver <- as.data.frame(matrix(c(0, 0, 0, 0), 2, 2, byrow = T)) # P of taking over a neighbiors position
...
P.TakeOver <- as.data.frame(matrix(c(0.4, 0, 0, 0.4), 2, 2, byrow = T)) # P of taking over a neighbiors position
names(P.TakeOver) <- c("Source.In.AppHabitat", "Source.NOT.In.AppHabitat") # How appropriate is the SOURCE's habitat to its strategy? lives
row.names(P.TakeOver) <- c("Target.In.AppHabitat", "Target.NOT.In.AppHabitat") # How appropriate is the TARGET society's habitat
