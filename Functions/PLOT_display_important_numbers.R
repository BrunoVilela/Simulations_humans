...
display_important_numbers_2 <- function(this_timestep){
...
head(data.result)
	...
	mtext(paste0(format(Sys.time(), format="%d %B %Y")), side=1, line=-9.5, adj=0)
	mtext("1254      Number of tips", side=1, line=-7, adj=0)
	mtext(paste0("1", "        Replicate"), side=1, line=-5, adj=0)
	mtext(paste0(this_timestep, "     Timesteps"), side=1, line=-3, adj=0)
