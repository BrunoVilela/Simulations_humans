...
##################################################################################
FarmSumPairwiseDistance <- function(which_model1, which_model2, which_model3, which_model4 ){
		
	colors <-  c("cornflowerblue", "firebrick", "darkgreen", "orange")
	
	data.result_sub_1 <- unlist(which_model1[[14]])
	data.result_sub_2 <- unlist(which_model2[[14]])
	data.result_sub_3 <- unlist(which_model3[[14]])
	data.result_sub_4 <- unlist(which_model4[[14]])
	
	par(mar=c(2,2,0,0))
	
plot(0,0, xlim=c(0,5), ylim=c(0, 20000000000), type="n", bty="n", xaxt="n", ylab="Branch lengths")
