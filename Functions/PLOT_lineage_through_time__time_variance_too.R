##################################################################################
LineageThroughTimeTvTwo <- function(which_model1, which_model2, which_model3, which_model4 ){
...
#lines(ltt_mean_a + ltt_SD_a, unique(both[,1]), lty=2, col=colors)
lines(ltt_mean_a, unique(both[,1]), lwd=3, col=adjustcolor(colors, alpha= 1))


}


plot(data.result_sub_1_x~ data.result_sub_1_y, type="n", xlim=c(1,6000), bty="n", ylab="lineages", xlab="time")

PlotLTT(data.result_sub_1_x, data.result_sub_1_y, colors[1])
#PlotLTT(data.result_sub_2_x, data.result_sub_2_y, colors[2])
PlotLTT(data.result_sub_3_x, data.result_sub_3_y, colors[2])
#PlotLTT(data.result_sub_4_x, data.result_sub_4_y, colors[4])


legend(0,1100,legend=c("Base model", "Base model + Takeover"), col=c( "limegreen", "orange"), lty=1, lwd=3)
