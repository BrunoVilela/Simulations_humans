...
EvolutionaryBirthDeathRates <- function(which_model1, which_model2, which_model3, which_model4 ){
	...
	data.result_sub_1 <- which_model1[[26]][1,]
	data.result_sub_2 <- which_model2[[26]][1,]
	data.result_sub_3 <- which_model3[[26]][1,]
	data.result_sub_4 <- which_model4[[26]][1,]
	
	par(mar=c(2,2,0,0))

	par(mar=c(2,2,0,0))
	plot(0,0, xlim=c(0,5), ylim=c(0, 2000), type="n", bty="n", xaxt="n", ylab="Branch lengths")
