...
Speciate <- function(myT, Parent, PosTargets, myWorld, mytree, NodeData, takeover) {
  ...
  if (is.null(mytree)) { 
    ...
    mytree <- read.tree(text = paste0("(t", Parent, ":", myT, ",t", NewSoc, ":", myT, ");")) 
    ...
    newtips <- read.tree(text = paste0("(t", Parent, ":", BL, ",t",NewSoc, ":", BL, ");")) 
    
    OldParentalNode <- NodeData$Node[which(NodeData$Tip == Parent)]
    
    mytree <- read.tree(text = write.tree(bind.tree(mytree, newtips, where = OldParentalNode),
                                          file = ''))
